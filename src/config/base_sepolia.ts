...
console.log("JBMultiTerminal for base_sepolia:", (jbContractAddress[5]?.JBMultiTerminal as any)?.[baseSepolia.id]);
console.log("JB721TiersHookStore for base_sepolia:", (jbContractAddress[5]?.JB721TiersHookStore as any)?.[baseSepolia.id]);
console.log("Base Sepolia chain ID:", baseSepolia.id);
console.log("JBMultiTerminal object:", jbContractAddress[5]?.JBMultiTerminal);

export const DEFIFA_CONFIG_BASE_SEPOLIA: DefifaConfig = {
  chainId: baseSepolia.id,

  // Use Juicebox v5 contracts from SDK
  JBProjects: {
    address: ((jbContractAddress[5]?.JBProjects as any)?.[baseSepolia.id] || "0x0000000000000000000000000000000000000000") as EthereumAddress,
    interface: jbProjectsAbi,
  },

  JBController: {
    address: ((jbContractAddress[5]?.JBController as any)?.[baseSepolia.id] || "0x0000000000000000000000000000000000000000") as EthereumAddress,
  ...
  JBETHPaymentTerminal: {
    address: ((jbContractAddress[5]?.JBMultiTerminal as any)?.[baseSepolia.id] || "0x0000000000000000000000000000000000000000") as EthereumAddress,
    interface: jbMultiTerminalAbi,
  },

  // Use v5 721 Tiers Hook instead of v3 delegate
  JBTiered721DelegateStore: {
    address: ((jbContractAddress[5]?.JB721TiersHookStore as any)?.[baseSepolia.id] || "0x0000000000000000000000000000000000000000") as EthereumAddress,
    interface: jb721TiersHookStoreAbi,
  },

  // JBRulesets contract for querying current ruleset/funding cycle
  JBRulesets: {
    address: ((jbContractAddress[5]?.JBRulesets as any)?.[baseSepolia.id] || "0x0000000000000000000000000000000000000000") as EthereumAddress,
    interface: jbRulesetsAbi,
  },

  // Defifa-specific contracts (v5 Base Sepolia deployments)
